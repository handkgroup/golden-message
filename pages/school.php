<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Golden Messaging | coaching</title>
  <link rel="stylesheet" href="../css/page/community.css" />
  <link rel="stylesheet" href="../css/page/school.css" />
</head>

<body>
  <!-- Navbar start -->
  <<header class="header-nav">
    <div class="header-nav_container">
      <img src="../img/common/logo.png" alt="logo" />
      <div class="burger-menu" id="burger-menu">
        <span></span>
        <span></span>
        <span></span>
      </div>
      <nav class="nav" id="nav-menu">
        <ul>
          <li><a class="list__link" href="./service.html">サービス</a></li>
          <li><a class="list__link" href="./about.html">会社概要</a></li>
          <li><a class="list__link" href="./purpose.html">ミッション</a></li>
          <li><a class="list__link" href="./news.html">お知らせ</a></li>
          <li><a class="list__link" href="./recruit.html">採用情報</a></li>
          <li><a class="list__link contact-btn" href="./contact.html">お問い合わせ</a></li>
        </ul>
      </nav>
    </div>
    </header>
    <!-- Navbar end -->

    <!-- Header start -->
    <header class="header" style="min-height: 525px;">
      <div class="container hero_container" style="padding: 60px 10px 50px 10%;">
        <span><a href="/pages/index.html">TOP　</a>>　サービス詳細</span>

        <h2>Reskilling School</h2>
        <p>学生コミュニティ事業</p>
      </div>
    </header>
    <!-- Header end -->

    <!-- Mission section start -->
    <section class="mission-section">
      <div class="container">
        <b class="section-title">MISSION</b>
        <h2 class="underlined">
          一人にしない。「やり遂げる」にこだわったコーチング型スクールを全ての人へ。
        </h2>
        <p>
          GoldenMessageでは、動画編集スクールをはじめ、小中学生向けの個別指導塾、高校生向けのコーチング型個別指導塾、営業マン養成スクールなど、様々なスクールサービスを展開しています。ゴール設定を明確にし、プロセスを徹底伴走して成し遂げる確かな運営ノウハウにより、累計2,800名（2025年時点）にご利用いただいています。これからも信頼と実績を積み重ね、従来の概念を超えたスクール事業を展開いたします。
        </p>
      </div>
    </section>
    <!-- Mission section end -->

    <!-- Feature section start -->
    <section class="propose-section schoolpropse-section">
      <div class="container">
        <b class="section-title">FEATURE</b>
        <h2>UNI MEETを企業様にオススメする理由</h2>

        <div class="cards schoolfeature-cards">
          <div class="card">
            <div class="content">
              <h3>動画編集スクール</h3>
              <p>
                本気で夢中になって打ち込めるものが無い。趣味や習い事も始めても続かない。毎日の生活にメリハリが足りず、惰性や、同じ毎日を繰り返している感覚が強い。
              </p>
            </div>
          </div>
          <div class="card">
            <div class="content">
              <h3>コーチング型個別指導塾</h3>
              <p>
                小学生・中学生・高校生を対象に、一人ひとりの特徴に合わせてモチベーションを引き出す個別指導の学習塾。最適な学習計画をつくり、専用のテキストを使用するるなど、目的に合わせてオーダーメイドされた指導法で、効果的な学習環境を作り、夢の実現をサポートします。
              </p>
            </div>
          </div>
          <div class="card">
            <div class="content">
              <h3>営業マン養成スクール</h3>
              <p>
                大学生や若手社会人を対象に、「実践重視」で営業力の向上を図ります。インプットした知識を、すぐに実践でアウトプットできるからこそ、確実にスキルが定着します。未経験向けの基礎～経験者向けの応用まで、一人ひとりに合わせたカリキュラムをご提供いたします。
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!-- Feature section end -->

    <!-- Staff section start -->
    <section class="feature-section school-staff">
      <div class="container">
        <b class="section-title">STAFF</b>
        <h2>GoldenMessageのスクール講師は一流の実力者揃い</h2>

        <div class="cards">
          <div class="card">
            <div class="content">
              <b class="school-b">
                ●厳しい採用基準をクリアした優秀な講師陣 <br>
                ●多様なバックグラウンドを持つからこそ、あなたに最適な講師を選定 </b>
              <p>
                <br>
                総勢2,700名以上の20代に向けたキャリアコーチングを提供。ファーストキャリアにおけるパフォーマンスの最大化を実現し、その先の転職や起業など、お客様が求める理想に向けて伴走します。一般的なカウンセリングやエージェントのアドバイザーでは実現できないクオリティや情報の質を担保し、大変ご満足いただいています。
              </p>
            </div>
            <div class="picture">
              <img src="../img/school/staff.png" alt="photo" />
            </div>
          </div>
        </div>
        <div class="school-bottom-text">
          <p>「やり遂げる」。最高の達成感をあなたに。</p>
        </div>
      </div>
    </section>
    <!-- Staff section end -->


    <!-- Contact section start -->
    <section class="contact-section">
      <div class="container">
        <b class="section-title">Contact</b>
        <h2>お問い合わせ</h2>
        <p>
          キャリアのモヤモヤをコーチングを通じて可能性に変えませんか？
          個人の方も企業のご担当者様もまずはお問い合わせください。
        </p>
        <a href="./contact.html"><button>お問い合わせする</button></a>
      </div>
    </section>

    <!-- Footer start -->
    <footer class="footer">
      <div class="container">
        <div>
          <a href="/index.html">
            <img src="../img/common/footer-logo.png" alt="LOGO" />
          </a>

          <div>
            <ul>
              <li>サービス</li>
              <li>サービス1</li>
              <li>サービス2</li>
              <li>サービス3</li>
              <li>サービス4</li>
            </ul>

            <ul>
              <li><a href="./about.html"><a href="./about.html">会社概要</a></a></li>
              <li><a href="./purpose.html">ミッション・バリュー</a></li>
              <li><a href="./news.html">お知らせ</a></li>
              <li><a href="./recruit.html">採用情報</a></li>
            </ul>

            <ul>
              <li><a href="./contact.html">お問い合わせ</a></li>
              <li><a href="#">個人情報保護方針</a></li>
            </ul>
          </div>
        </div>

        <p>
          <a href="/pages/policy.html">プライバシーポリシー</a> <span>© copyright 2024 Golden Message</span>
        </p>
      </div>
    </footer>
    <!-- Footer end -->
    <script src="../js/script.js"></script>
</body>

</html>